...
def get_test_imgclient():
    return image.IIIFImageClient(api_endpoint=api_endpoint, image_id=image_id)
...
class TestIIIFImageClient:
    ...
    def test_init_opts(self):
        test_opts = {
            "region": "2560,2560,256,256",
            "size": "256,",
            "rotation": "90",
            "quality": "color",
            "fmt": "png",
        }
        img = image.IIIFImageClient(
            api_endpoint=api_endpoint, image_id=image_id, **test_opts
        )
    ...
    def test_image_width_height(self):
        ...
        with patch.object(image.IIIFImageClient, "image_info", new=sample_image_info):
    ...
    def test_canonicalize(self):
        ...
        with patch.object(image.IIIFImageClient, "image_info", new=square_img_info):
...
class TestImageRegion:
    ...
    def test_init(self):
        ...
        # percentage region
        region = image.ImageRegion(x=5, y=7, width=100, height=103, percent=True)
        assert region.as_dict()["full"] is False
        assert region.as_dict()["x"] == 5
        assert region.as_dict()["y"] == 7
        assert region.as_dict()["width"] == 100
    ...
    def test_render(self):
        ...
        region = image.ImageRegion(x=5, y=5, width=100, height=100, percent=True)
        assert str(region) == "pct:5,5,100,100"
        region = image.ImageRegion(
            x=5.1, y=3.14, width=100.76, height=100.89, percent=True
        )
    ...
    def test_canonicalize(self):
        ...
        with patch.object(image.IIIFImageClient, "image_info", new=tall_img_info):
        ...
        with patch.object(image.IIIFImageClient, "image_info", new=wide_img_info):
            # square requested, should convert to x,y,w,h
            img.region.parse("square")
            img.region.canonicalize()
...
class TestImageSize:
    ...
    def test_canonicalize(self):
        ...
        with patch.object(image.IIIFImageClient, "image_info", new=rect_img_info):
            img.size.parse("!50,50")
            img.size.canonicalize()
